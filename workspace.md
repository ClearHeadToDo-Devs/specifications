...
A root without `charters/README.md` or without its `id` still loads as the single root. Like a workspace without a `workspace_id`, it gets an ephemeral identity that is never persisted and never derived from a path, and its display name falls back to `workspace_name`. A root with no README, or a README that omits `state`, is `New` like any [charter](./charters.md#frontmatter) without a declared state; because engagement requires every ancestor to be `Active`, nothing in the workspace is admitted for engagement until the root is activated. `clearhead init` fills in missing root files and writes the root as `New`, since a workspace starts in planning; `doctor` reports the gap; loading never writes identity or state. When the README and sidecar disagree, the README wins: `doctor --fix` mirrors its `id` into the sidecar only when nothing references the replaced id, and otherwise reports a violation to reconcile by hand.
...
1. the subtree's known files (`.actions`, `.completed.actions`, `.upcoming.actions`, the charter `.md`, and its `.json` sidecar) plus all supporting files owned by directory-form charters are moved into the `archive/` region together, preserving their path under `charters/` so the subtree's internal structure survives. The sidecar moves *with* the files rather than folding its `created_at` / `external_schedule_id` into the lines. An implementation must make the move safe to interrupt and retry: a file may transiently exist in both `charters/` and `archive/`, but it is never removed from `charters/` without already existing in `archive/` — redundant, never an orphaned or lost record
