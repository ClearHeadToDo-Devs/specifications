...
1. [Action File Format](./action_file_format.md) - The format for `.actions` files that represent planned acts (execution records). If you read nothing else, start here to understand how planned acts are defined.
...
2. [ICS Schedule Specification](./ics_schedule_spec.md) - Defines how `.ics` files represent plans/schedules (VEVENT-based timing) and how schedule instances map into generated planned acts.
