...
2. [ICS Schedule Specification](./ics_schedule_spec.md) - Defines how `.ics` files represent plans/schedules (VEVENT-based timing) and how schedule instances map into generated actions.
...
7. [Query Output](./query_output.md) - The single JSON-LD contract for query and view results, consumed identically by UI clients and integration partners. Defines canonical identity (semantic `@id`, compacted client-facing `id`), the `SELECT`/`CONSTRUCT` rule, ordering discipline, and the stateless-producer + verbs-by-identity seam.
    1. A client (for example [clearhead.nvim](https://github.com/ClearHeadToDo-Devs/clearhead.nvim)) reads the `@graph` array and maps each node to its own widget (a quickfix list), while an integration partner honors the `@context` — from the same payload.
