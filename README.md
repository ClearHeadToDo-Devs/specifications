...
7. [Query Output](./query_output.md) - The single JSON-LD contract for query and view results, consumed identically by UI clients and integration partners. Defines `@id` identity, the `SELECT`/`CONSTRUCT` rule, ordering discipline, and the stateless-producer + verbs-by-`@id` seam.
